...
curl -X POST "http://localhost:8084/produce" -H "Content-Type: text/plain" -d "Hello World!"


# Para enviar uma mensagem JSON 
curl -X POST http://localhost:8084/produce -H "Content-Type: application/json" -d '{"message": "Hello World"}'
...
# Testar pelo método POST
curl -X POST "http://localhost:8080/produce" -H "Content-Type: text/plain" -d "Hello World!"
